...
class Ui_SAMprototype(object):
    ...
    totalImages=None
    currImage=0
    sam_checkpoint = "sam_vit_h_4b8939.pth" #Enter actual download path
    device="cpu"
    model_type = "default"
    ...
    def getPos(self , event):
        xCord = event.pos().x()
        yCord = event.pos().y()
        self.xCoord.display(x)
        self.yCoord.display(y)
        input_point = np.array([[xCord,yCord]])
        input_label = np.array([1])
        self.masks, self.scores, self.logits = self.predictor.predict(point_coords = input_point,
                  point_labels = input_label,
                  multimask_output=True)
        for _, (mask, score) in enumerate(zip(self.masks, self.scores)):
        ...
        rect = patches.Rectangle((x, y), height=height-y, width=width-x, linewidth=1, edgecolor='g', facecolor='none')
        ax.add_patch(rect)
